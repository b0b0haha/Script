...
def query_raw_bytes():
    try:
        ...
        print e
        return []
#============================ for get the diff_function mapping==============================#
def add_ImageMagick_match(cur_dot_url, par_dot_url,vs_dot_url, cur_svg_url, par_svg_url, vs_svg_url,function_address, cur_tag, par_tag,mapping_json):
    try:
        conn = connect_to_db_simple()
        cursor = conn.cursor()

        sql = "select id from S_ImageMagick_Match where cur_dot_url = '%s' and  par_dot_url = '%s' and  vs_dot_url = '%s' and  cur_svg_url = '%s' and par_svg_url = '%s' and vs_svg_url = '%s' and  function_address = '%s' and  cur_tag = '%s' and  par_tag ='%s';"\
         % (str(cur_dot_url), str(par_dot_url), str(vs_dot_url), str(cur_svg_url), str(par_svg_url), str(vs_svg_url), str(function_address), str(cur_tag), str(par_tag))
        cursor.execute(sql)
        result = cursor.fetchall()
        # print result
        if len(result) ==0 :
        ...
        sql = "insert into S_ImageMagick_Match (cur_dot_url,par_dot_url,vs_dot_url,cur_svg_url,par_svg_url,vs_svg_url,function_address,cur_tag,par_tag,mapping_json) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s');" \
              % (str(cur_dot_url), str(par_dot_url), str(vs_dot_url), str(cur_svg_url), str(par_svg_url), str(vs_svg_url), str(function_address), str(cur_tag), str(par_tag), str(mapping_json))
        cursor.execute(sql)
        ...
        print e
        return -1
def add_ImageMagick_FileInfo(commitID,parentID,cur_file,par_file,filename,cur_func_addre,par_func_addre):
    try:
        conn = connect_to_db_simple()
        sql = "select id from S_ImageMagick_Fileinfo where commitID = '%s'and parentID = '%s'and cur_file= '%s' and par_file = '%s' and filename = '%s'and cur_func_addre ='%s'and par_func_addre= '%s';"\
        %(commitID,parentID,cur_file,par_file,filename,cur_func_addre,par_func_addre)
        cursor = conn.cursor()
        cursor.execute(sql)
        result = cursor.fetchall()
        if len(result)>0 :
            return False
        sql = "insert into S_ImageMagick_Fileinfo (commitID,parentID,cur_file,par_file,filename,cur_func_addre,par_func_addre)values('%s','%s','%s','%s','%s','%s','%s');"\
        %(commitID,parentID,cur_file,par_file,filename,cur_func_addre,par_func_addre)
        cursor.execute(sql)
        conn.commit()
        conn.close()
...
def get_ImageMagick_FileInfo():
    try:
        conn = connect_to_db_simple()
        sql = "select * from S_ImageMagick_Fileinfo;"
        cursor = conn.cursor()
        cursor.execute(sql)
        result = cursor.fetchall()
        conn.commit()
...
def query_fileID_by_fun(path,functiom_name):
    try:
        conn=connect_to_db_simple()
        sql="select debug_file, function_address from S_ImageMagick_Src2Bin where source_file = '%s' and function_name = '%s';" % (path, functiom_name)
        cursor = conn.cursor()
        cursor.execute(sql)
        result = cursor.fetchall()
        conn.close()
...
def query_filepath_by_fileID(debug_file, commit_id):
    try:
        conn=connect_to_db_simple()
        sql="select local_url,filename from S_ImageMagick_Bin where id = '%s' and tag = '%s';" %(str(debug_file), str(commit_id))
        cursor=conn.cursor()
        cursor.execute(sql)
        result=cursor.fetchall()
        conn.close()
...
def get_commit_version_snapshots(repo, start, limit):
    try:
        conn = connect_to_db_simple()
        sql ="select commit,local_url from Security_Commit_Snapshot where limit '%d' '%d'"%(start.limit)
        cursor = conn.cursor()
        cursor.execute(sql)
        result = cursor.fetchall()
        conn.close()
...
def get_processed_commit_version_snapshots_openssl():
    try:
        conn = connect_to_db_simple()
        sql = "select distinct tag from S_OpenSSL_Bin;"
        cursor = conn.cursor()
        cursor.execute(sql)
        result = cursor.fetchall()
        conn.close()
        ...
        print e
        return []
def get_processed_commit_version_snapshots_phpsrc():
    try:
        conn = connect_to_db_simple()
        sql = "select distinct tag from S_Php-src_Bin;"
        cursor = conn.cursor()
        cursor.execute(sql)
        result = cursor.fetchall()
        conn.close()
...
def add_ImageMagick_bin(uri, filename, tag):
    try:
        ...
        print e
        return -1
def add_phpsrc_bin(uri, filename, tag):
    try:
        conn = connect_to_db_simple()
        cursor = conn.cursor()

        sql = "select id from S_Php-src_Bin where tag = '%s' and filename = '%s';" % (tag, filename)
        cursor.execute(sql)
        result = cursor.fetchall()
        # print result
        if len(result) > 0:
        ...
        sql = "insert into S_Php-src_Bin (local_url, filename, tag) values ('%s', '%s', '%s');" \
              % (uri, filename, tag)
        cursor.execute(sql)
...
def query_ImageMagick_compile_files():
    try:
        conn = connect_to_db_simple()
        ...
        sql = "SELECT id, local_url, filename, tag FROM `S_ImageMagick_Bin`;"
        cursor.execute(sql)
        result = cursor.fetchall()
...
def query_commit_parents(commit):
    try:
...
def query_commit_tag(repo, commit):
    try:
...
def query_all_buildlog():
    try:
        ...
        sql = "select id, buildlog_local_url from U_Build where buildlog_local_url != 'None' and id not in (select distinct build from U_Build_Option);"
        cursor = conn.cursor()
        cursor.execute(sql)
        result = cursor.fetchall()
        conn.close()
